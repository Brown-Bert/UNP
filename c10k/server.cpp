...
const my_size_t LOWLEVEL = TASKSQUEUESIZE * 40 / 100; // 任务低潮标志，当任务个数低于这个标志的时候就要减少线程数目，防止太多的线程空转，浪费系统资源
const my_size_t HIGHLEVEL = TASKSQUEUESIZE * 90 / 100; // 任务高潮标志，当任务队列中任务个数大于这个标志时，就要增加线程个数，防止任务过多把任务队列给撑爆
    ...
    while (true) {
        if (CURRENTTASKCOUNT == 0 && SHUTDOWNFLAG == 1)
        {
            ...
            if (CURRENTTHREADCOUNT > THREADLOWCOUNT)
            {
                ...
                // std::cin.ignore(std::numeric_limits<std::streamsize>::max()); // 清空输入缓冲区
                my_size_t f = 0;
                while (input.empty()) {
...
int main(int argc, char* argv[])
{
    ...
    while (true) 
    {
        ...
        if (CURRENTTASKCOUNT > HIGHLEVEL)
        {
            pthread_cond_signal(&cond_notify_becauseofhigh); // 此时任务数量大于了高潮标志，所以需要通知checkHigh线程去增加线程消费者的数量
        }
        if (CURRENTTASKCOUNT > LOWLEVEL)
        {
